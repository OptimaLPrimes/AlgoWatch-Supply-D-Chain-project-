import type { Batch, Checkpoint, TemperatureLog, UserRole, DashboardOverviewCardData } from '@/types';
...
export const mockBatches: Batch[] = [
  {
    ...
    qrCodeUrl: 'https://placehold.co/150x150.png?text=QR+BATCH001',
    attachments: [
      { name: 'Invoice_001.pdf', url: '#', type: 'invoice' },
      { name: 'Product_Image.jpg', url: 'https://placehold.co/300x200.png', type: 'image' }
    ],
    checkpoints: [
  ...
  },
  {
    ...
    qrCodeUrl: 'https://placehold.co/150x150.png?text=QR+BATCH002',
    checkpoints: [
  ...
  },
  {
    ...
    qrCodeUrl: 'https://placehold.co/150x150.png?text=QR+BATCH003',
    checkpoints: [
...
export const mockDashboardCards: (role: UserRole) => DashboardOverviewCardData[] = (role) => {
  const baseCards = [
    { title: 'Total Batches', value: mockBatches.length, icon: Package, trend: '+2 this week', bgColorClass: 'bg-blue-100 dark:bg-blue-900', textColorClass: 'text-blue-600 dark:text-blue-300' },
    { title: 'Deliveries In Transit', value: mockBatches.filter(b => b.status === 'In Transit').length, icon: Truck, trend: '1 new today', bgColorClass: 'bg-yellow-100 dark:bg-yellow-900', textColorClass: 'text-yellow-600 dark:text-yellow-300' },
    { title: 'Completed Deliveries', value: mockBatches.filter(b => b.status === 'Delivered').length, icon: CheckCircle2, trend: '+10% vs last month', bgColorClass: 'bg-green-100 dark:bg-green-900', textColorClass: 'text-green-600 dark:text-green-300' },
    { title: 'Active Alerts', value: 1, icon: AlertTriangle, trend: 'Temp. breach BATCH001 (simulated)', bgColorClass: 'bg-red-100 dark:bg-red-900', textColorClass: 'text-red-600 dark:text-red-300' },
  ];

  if (role === 'Admin') {
    return [
      ...baseCards,
      { title: 'Registered Users', value: 25, icon: UserCircle, trend: '+3 new users', bgColorClass: 'bg-indigo-100 dark:bg-indigo-900', textColorClass: 'text-indigo-600 dark:text-indigo-300' },
      { title: 'Smart Contract Txs', value: 152, icon: ShieldCheck, trend: 'Avg. 5/day', bgColorClass: 'bg-purple-100 dark:bg-purple-900', textColorClass: 'text-purple-600 dark:text-purple-300' },
    ];
  }
  if (role === 'Manufacturer') {
     return [
      { title: 'Batches Created', value: mockBatches.filter(b => b.checkpoints[0]?.handlerRole === 'Manufacturer').length, icon: Factory, trend: '+1 this week', bgColorClass: 'bg-sky-100 dark:bg-sky-900', textColorClass: 'text-sky-600 dark:text-sky-300' },
      ...baseCards.slice(1,3) // In Transit, Completed
    ];
  }
   if (role === 'Distributor') {
     return [
      { title: 'Batches Handled', value: mockBatches.filter(b => b.checkpoints.some(c => c.handlerRole === 'Distributor')).length, icon: Warehouse, trend: '2 awaiting pickup', bgColorClass: 'bg-teal-100 dark:bg-teal-900', textColorClass: 'text-teal-600 dark:text-teal-300' },
      ...baseCards.slice(1,3)
    ];
  }
  if (role === 'Retailer') {
     return [
      { title: 'Batches Received', value: mockBatches.filter(b => b.status === 'Delivered' && b.checkpoints.some(c => c.handlerRole === 'Retailer')).length, icon: Store, trend: '1 pending verification', bgColorClass: 'bg-lime-100 dark:bg-lime-900', textColorClass: 'text-lime-600 dark:text-lime-300' },
